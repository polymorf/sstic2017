...
function FS() {
    ...
    message.Register("FilesInDirectory", this.FilesInDirectory.bind(this) );
    ...
    message.Register("CreateDirectory", 
        function(dirinfo){
            var ids = this.SearchPath(dirinfo.name);
            if(ids.id == -1 && ids.parentid != -1) {
                var dir_id = this.CreateDirectory(ids.name, ids.parentid);
                var x = this.GetInode(dir_id);
                if (dirinfo.hasOwnProperty("atime")) {
                    x.atime = dirinfo.atime;
                }
                if (dirinfo.hasOwnProperty("mtime")) {
                    x.mtime = dirinfo.mtime;
                }
                if (dirinfo.hasOwnProperty("ctime")) {
                    x.ctime = dirinfo.ctime;
                }
                if (dirinfo.hasOwnProperty("uid")) {
                    x.uid = dirinfo.uid;
                }
                if (dirinfo.hasOwnProperty("gid")) {
                    x.gid = dirinfo.gid;
                }
                if (dirinfo.hasOwnProperty("mode")) {
                    x.mode = dirinfo.mode;
                }
            }
        }.bind(this)
    ...
    message.Register("FilesInDirectory",
        function(file) {
            message.Send("FilesInDirectory", (this.FilesInDirectory.bind(this))(file));
        }.bind(this)
    );
...
FS.prototype.LoadFile = function(idx) {
    ...
    (function(idx){
        return function(buffer){
            ...
            if (inode.size != inode.data.length) message.Warning("Size wrong for uncompressed non-lazily loaded file: " + inode.name +" "+inode.size+" should be "+inode.data.length);
...
FS.prototype.CreateDirectory = function(name, parentid, dirinfo) {
...
FS.prototype.ReadFile = function(file) {
    ...
    file.atime = this.inodes[ids.id].atime;
    file.mtime = this.inodes[ids.id].mtime;
    file.ctime = this.inodes[ids.id].ctime;
    file.uid = this.inodes[ids.id].uid;
    file.gid = this.inodes[ids.id].gid;
    file.mode = this.inodes[ids.id].mode;
    file.mode = this.inodes[ids.id].mode;
    file.url = this.inodes[ids.id].url;
...
FS.prototype.MergeFile = function(file) {
    ...
    var ids = this.SearchPath(file.name);
    if (ids.parentid == -1) {
        //console.log("ICI TODO"+file.name);
        return; // not even the path seems to exist
    }
    if (ids.id == -1) {
    ...
    if (file.hasOwnProperty("atime")) {
        this.inodes[ids.id].atime = file.atime;
    }
    if (file.hasOwnProperty("mtime")) {
        this.inodes[ids.id].mtime = file.mtime;
    }
    if (file.hasOwnProperty("ctime")) {
        this.inodes[ids.id].ctime = file.ctime;
    }
    if (file.hasOwnProperty("uid")) {
        this.inodes[ids.id].uid = file.uid;
    }
    if (file.hasOwnProperty("gid")) {
        this.inodes[ids.id].gid = file.gid;
    }
    if (file.hasOwnProperty("mode")) {
        this.inodes[ids.id].mode = file.mode;
    }
}

FS.prototype.FilesInDirectory = function(file) {
    var files = []
    var inids = []
    var ids = this.SearchPath(file.name);
    if (ids.parentid == -1 || ids.id == -1) return;
    
    this.GetRecursiveList(ids.id, inids);
    for(var i=0; i<inids.length; i++) {
        files.push(this.GetFullPath(inids[i]));
    }
    file.files = files;
    return file;
}
...
FS.prototype.NotifyListeners = function(id, action, info) {
    ...
    if (path.startsWith("home/user") || path.startsWith("challenges") || path.startsWith("root") ) {
        message.Send("WatchAllFSEvent", {path: path, event: action, info: info});
    }
